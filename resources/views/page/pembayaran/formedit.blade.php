...
@section('content')
  ...
  'method' => 'post',
  'route' => 'pembayaran.update',$idbayar->id_bayar
]) !!} --}}
                            ...
                            <div class="col-md-6">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        ...
                                        id="id_bank" style="width:100%">
                                    
                                        <!-- Opsi default -->
                                        <option value="0">Pilih</option>
                                    
                                        @foreach($bank as $bnk)
                                            @if ($bnk->id_bank == $idbayar->spmbayar_bank_id)
                                                <!-- Opsi yang dipilih berdasarkan data dari database -->
                                                <option value="{{ $bnk->id_bank }}" data-id="{{ $bnk->id_bank }}" selected>
                                                    {{ $bnk->nama_bank }}-{{ $bnk->deskripsi }}
                                                </option>
                                            @else
                                                <!-- Opsi yang tidak dipilih -->
                                                <option value="{{ $bnk->id_bank }}" data-id="{{ $bnk->id_bank }}">
                                                    {{ $bnk->nama_bank }}-{{ $bnk->deskripsi }}
                                                </option>
                                            @endif
                                        @endforeach
                                ...
                                <div class="form-group row">
                                    <label for="unit" class="col-sm-4 col-form-label">Tanggal bayar </label>
                                    <div class="col-sm-8">
                                        <input type="date" class="form-control form-control-sm" name="spmbayar_tgl_bayar" value="{{ $idbayar->spmbayar_tgl_bayar }}"  placeholder="tgl bayar" >
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="unit" class="col-sm-4 col-form-label">Catatan</label>
                                    <div class="col-sm-8">
                                        <input type="text" class="form-control  form-control-sm" name="spmbayar_catatan" value="{{ $idbayar->spmbayar_catatan }}" placeholder="catatan" >
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <input class="form-control  form-control-sm" id="spmbayar_nominal" NAME="spmbayar_nominal" value="{{$idbayar->spmbayar_nominal}}" type="number" placeholder="nominal">
                                   </div>
                                </div>
                                <div class="form-group row">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        ...
                                        @foreach($project as $prj)
                                            @if ($prj->project_no_char == $idbayar->id_project)
                                                <!-- Opsi yang dipilih berdasarkan data dari database -->
                                                <option value="{{ $prj->project_no_char }}" data-id="{{ $prj->project_no_char }}" selected>
                                                    {{ $prj->project_name }}
                                                </option>
                                            @else
                                                <!-- Opsi yang tidak dipilih -->
                                                <option value="{{ $prj->project_no_char }}" data-id="{{ $prj->project_no_char }}">
                                                    {{ $prj->project_name }}
                                                </option>
                                            @endif
                                        @endforeach
                                ...
                                <table class="table table-bordered table-hover dataTable" id="listpinjam">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{$item->spmbayar_nochar}}</td>
                                            <td>buat nama </td>
                                            <td>{{$item->jenis_spmhd}}</td>
                                            <td>{{$item->spmdt_invoice}}</td>
                                           
                                            <td>{{$item->spmdt_tgl_bayar}}</td>
                                            <td>{{$item->spmdt_nominal}}</td>
                                            <td></td>
                                        </tr>
                                        @endforeach
